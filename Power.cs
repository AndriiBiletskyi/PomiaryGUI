...
namespace PomiaryGUI
{
    public partial class Power : UserControl
    {
        ...
        private List<string> GetChekedLines()
        {
        ...
        public void powerchart(DataTable table, string _mode)
        {
            if (_mode == "power")
            {
                this.checkP.Text = "P";
            ...
            try
            {
                ...
                if (table.Rows.Count > dots)
                {
                    for (Int32 i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        {
                            temptable.Clear();
                            for (Int32 q = 0; q < (table.Rows.Count / dots); q++)
                            {
                                ...
                                if (Convert.ToBoolean(table.Rows[i]["Status"]))
                                {
                                    foreach (string str in GetChekedLines())
                                    {
                                        if(_mode=="power")temprow[str] = Convert.ToDouble(table.Rows[i][str]) / 1000;
                        ...
                        else
                        {
                            if (Convert.ToBoolean(table.Rows[i]["Status"]))
                            {
                                foreach (string str in GetChekedLines())
                                {
                                    if (_mode == "power") Lines[GetChekedLines().IndexOf(str)].Add(Convert.ToDouble(table.Rows[i][str]) / 1000);
                ...
                else
                {
                    foreach (DataRow row in table.Rows)
                    {
                        if (Convert.ToBoolean(row["Status"]))
                        {
                            foreach (string str in GetChekedLines())
                            {
                                if (_mode == "power") Lines[GetChekedLines().IndexOf(str)].Add(Convert.ToDouble(row[str]) / 1000);
